...
// Blends an effect result (InSampler) over the untouched frame (OrigSampler) inside up to 4
// screen circles. Each circle: xy = centre in pixels (origin top-left), z = radius in pixels,
// w = strength 0..1. Unused circles have z = 0 and are skipped.
...
void main() {
    ...
    float mask = max(max(circle(C0, pixel), circle(C1, pixel)), max(circle(C2, pixel), circle(C3, pixel)));
